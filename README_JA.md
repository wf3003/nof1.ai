...
- **ポジション追加/補填の適応**: 利益時は50%以内の追加、損失時は30%以内の補填、レバレッジは既存ポジションに自動的に合わせる
- **流動性不足時の自動縮小**: 板の厚みが不足した場合、数量を20%ずつ自動削減（最大5回試行）
- **3段階ドローダウン保護**: 20%超過で警告、30%超過で新規ポジション禁止、50%超過で全ポジション強制決済
